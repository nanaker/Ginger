...
df = pd.read_csv('../../dataset/MIM/MIM_train_result2.csv')
...
initial=(np.nan,np.nan,df_naive_bayes['F_mesure'].iloc[0],np.nan)
print(initial)
...
AllKNN=(df_decision_tree['F_mesure'].iloc[2],np.nan,df_naive_bayes['F_mesure'].iloc[2],df_svm['F_mesure'].iloc[2])
print(AllKNN)
InstanceHardnessThreshold=(df_decision_tree['F_mesure'].iloc[3],np.nan,df_naive_bayes['F_mesure'].iloc[3],df_svm['F_mesure'].iloc[3])
print(InstanceHardnessThreshold)
OneSidedSelection=(np.nan,np.nan,df_naive_bayes['F_mesure'].iloc[5],np.nan)
print(OneSidedSelection)
TomekLinks=(np.nan,np.nan,df_naive_bayes['F_mesure'].iloc[6],np.nan)
print(TomekLinks)
...
rects4 = ax.bar(ind + width*4, OneSidedSelection, width,
                color=cmap(0.8), label='OneSidedSelection')

rects5 = ax.bar(ind + width*5, TomekLinks, width,
                color=cmap(0.9), label='TomekLinks')


# Add some text for labels, title and custom x-axis tick labels, etc.
ax.set_ylabel('F_mesure')
#ax.set_title('Résultat de classification  par dataset et par Algorithme du défaut de code MIM')
