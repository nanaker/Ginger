...
data_full_name = pd.read_csv('dataset/MIM/prediction/MIM_prediction.csv')

data=pd.read_csv('dataset/MIM/prediction/MIM_prediction.csv')
data.drop('full_name', axis=1, inplace=True)
data.drop('number_of_instructions', axis=1, inplace=True)
...
data2.to_csv('dataset/MIM/prediction_result2.csv', index=False)
