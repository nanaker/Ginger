...
public class LICQuery extends Query {
    ...
    protected String getQuery(boolean details) {
        String query = "MATCH (cl:Class) WHERE  exists(cl.is_inner_class) RETURN CASE WHEN cl.is_static = true " +
                "THEN true ELSE false END AS is_static ,cl.is_enum as is_enum " +
                ...
                "THEN false ELSE true END as call_method,CASE WHEN cl.is_interface = true THEN true ELSE false END AS is_interface " +
                ",cl.is_local_class as is_local_class,cl.class_complexity as class_complexity  ";
        if (details) {
